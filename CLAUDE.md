...
- **Numeric fields are placeholders when there's no real activity.** Accounts with `ftd_received=false` AND `ftt_made=false` come back with `deposit_amount=1`, `client_balance=1`, `client_equity=1`, `ftd_amount=1` — *not* zero. `fetch_snapshot()` zeroes `deposit_total`/`balance` when neither flag is set. Trust the boolean flags (`kyc_passed`, `ftd_received`, `ftt_made`), not the numbers.
...
- `fetch_snapshot()` returns `None` on any transient error (network, 5xx, bad JSON), an "empty" snapshot (`under_partner=False`) on a definitive miss, or a full `ClientSnapshot`. Activation: `is_activated(progress_flags, deposit_total)` requires `ftd_received` AND `deposit_total ≥ MIN_DEPOSIT_USD` (the bare `ftt_made` path was removed — a no-deposit bonus trade must not grant access; `ftd_received` is sticky on Exness's side once true, so a real depositor never trips this on re-check). If `ACTIVATION_REQUIRE_TRADE=true`, also requires `ftt_made`. `recheck_verified_users` re-evaluates `is_activated` and kicks (`kicked_not_activated`) accounts that no longer pass. `progress_flags` is a synthetic list built from the row's bool fields (with legacy `contact_sharing_progress_status` list support).
...
2. `recheck_verified_users` every `RECHECK_INTERVAL_HOURS` — for `verified`/`warned`. Kick triggers: not under partner / `client_status in (LEFT, CHANGING)` → immediate; `ftd_received` flag present AND `balance < 1` → withdrew everything; inactivity past `INACTIVITY_WARN_DAYS` → warn, then kick after `WARNING_GRACE_DAYS` if still inactive (recovers if active again). **Inactivity logic is skipped entirely when there's no `last_trade_at` timestamp** — a deposit-only user must never be kicked for "no trades".
